...
@Command(name = "GAVFind", mixinStandardHelpOptions = true, version = "GAVFind 0.1",
...
public class GAVFind implements Callable<Integer> {
    ...
    private static final Pattern GAV_PATTERN = Pattern.compile("^(?<group>[a-z0-9._-]+):(?<artifact>[a-z0-9._-]+)(?::(?<version>[a-z0-9._-]+))?$", Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
    private static final Pattern GH_PR_TITLE_PATTERN = Pattern.compile("^build\\(deps\\): bump (?<ga>[a-z0-9:._-]+) from (?:[a-z0-9._-]+) to (?<version>[a-z0-9._-]+)$", Pattern.MULTILINE);

    @Parameters(index = "0", description = "The Maven dependency GroupId:ArtifactId[:Version] (GAV), ex. org.slf4j:slf4j-api:2.0.12 or info.picocli:picocli . If no version is specified (version listing mode) then all available versions are printed, otherwise the existence of the specified version is checked. If the GitHub 'gh' client is installed, this can also be a GitHub Pull Request path and the PR title will be used to infer the GAV.")
    private String gav;

    @Option(names = { "-r", "--repository" }, description = "The Maven repository root URL to search, ex. https://repo.maven.apache.org/maven2/", defaultValue = "https://repo.maven.apache.org/maven2/")
    private String repoRoot;

    @Option(names = { "-c", "--count" }, description = "The number of release versions to list in version listing mode. Defaults to the full list", defaultValue = "-1")
    private int count;

    @Option(names = { "-k", "--insecure" }, description = "Disable TLS validation", defaultValue = "false")
    private boolean insecure;

    @Option(names = { "-v", "--verbose" }, description = "Enable verbose debugging output", defaultValue = "false")
    ...
    public Integer call() throws Exception {
        ...
        if (exactMatch) {
            System.out.println(String.format("Searching %s for version \"%s\" of %s from %s", repoRoot, version, artifactId, groupId));
        } else {
            System.out.println(String.format("Searching %s for available versions of %s from %s", repoRoot, artifactId, groupId));
        }

        var url = String.format("%s/%s/%s/maven-metadata.xml", repoRoot, groupId.replaceAll("\\.", "/"), artifactId);
        ...
        if (exactMatch) {
            ...
            if (versionMatch.isPresent()) {
                System.out.println(String.format("%s:%s:%s is available as %s in %s", groupId, artifactId, version, versionMatch.get(), repoRoot));
            } else {
                System.err.println(String.format("%s:%s:%s is NOT available in %s", groupId, artifactId, version, repoRoot));
                System.err.println("available:");
                versioning.versions().stream().limit(count < 0 ? Integer.MAX_VALUE : count).map(v -> "\t" + v).forEach(System.err::println);
                return 2;
            }
        } else {
            System.out.println(String.format("latest: %s\n", versioning.latest()));
            System.out.println(String.format("release: %s\n", versioning.release()));
            System.out.println("available:");
            versioning.versions().stream().limit(count < 0 ? Integer.MAX_VALUE : count).map(v -> "\t" + v).forEach(System.out::println);
        }
    ...
    private static Versioning parseVersioning(Element metadata) {
        ...
        var versionList = new ArrayList<>(getChildren(versions, "version").stream().map(Node::getTextContent).toList());
    ...
    private String extractGitHubTitleGav(String url) throws IOException, InterruptedException {
        ...
        if (!matcher.matches()) {
            throw new RuntimeException(String.format("GitHub PR URL \"%s\" was not understandable. Got title: \"%s\"", url, proc.out().get(0)));
        }
